...
module ControlPartsS (addressSelectLogic, microMemory, aluControl) where
...
dispatch1 = do
	...
	(ni, no) <- notGate0
	connectWire (instout, 1, 4) (ni, 1, 0)
	connectWire0 no nstin
	b1 <- constGate0 $ Bits 0
	connectWire (b1, 1, 0) (nstin, 1, 1)
	(x1, x2, xo) <- xorGate0
	connectWire (instout, 1, 4) (x1, 1, 0)
	connectWire (instout, 1, 6) (x2, 1, 0)
	connectWire (xo, 1, 0) (nstin, 1, 2)
...
dispatch2 = do
	(instin, instout) <- idGate64
	(nstin, nstout) <- idGate64
	connectWire (instout, 1, 5) (nstin, 1, 0)
	b1 <- constGate0 $ Bits 1
	connectWire (b1, 1, 0) (nstin, 1, 1)
	b2 <- constGate0 $ Bits 0
	connectWire (b2, 1, 0) (nstin, 1, 2)
	return (instin, nstout)
...
checkStop = pla8 $ zip [0 ..] [1, 1, 0, 0, 0, 0]
